                                ...
                                <div class="row">
                                    <div class="col-12" id="botoes-acoes">
                                        <div id="botoes-parte-1">
                                            ...
                                            <button data-action="COD_EMAIL" onclick="mostrarFormularioEmail();" type="button" class="btn btn-outline-info btn-sm ml-2 btn-action"><i class="fas fa-envelope"></i> COD EMAIL</button>
                                            <br>
                                            ...
                                            <div id="formulario-email" style="display: none; margin-top: 10px; padding: 15px; border: 1px solid #ccc; border-radius: 5px; background-color: #f8f9fa;">
                                                <div class="form-group">
                                                    <input type="email" id="email-cliente" class="form-control" placeholder="Digite o e-mail do cliente" style="margin-bottom: 10px;">
                                                    <button onclick="enviarCodigoEmail();" type="button" class="btn btn-primary btn-sm">
                                                        <i class="fas fa-paper-plane"></i> Enviar código
                                                    </button>
                                                    <button onclick="fecharFormularioEmail();" type="button" class="btn btn-secondary btn-sm ml-2">
                                                        <i class="fas fa-times"></i> Cancelar
                                                    </button>
                                                </div>
                                                <div id="mensagem-email" style="margin-top: 10px; font-weight: bold; color: #28a745;"></div>
                                            </div>
...
<script>
    ...
    function fecharFormularioEmail() {
    ...
    function enviarCodigoEmail() {
        var emailCliente = document.getElementById('email-cliente').value;
        ...
        if (confirm("Deseja mesmo enviar código para: " + emailCliente + "?")) {
            // Exibir mensagem de confirmação
            document.getElementById('mensagem-email').innerHTML = 'Código será enviado para: ' + emailCliente;
            
            // Solicitar comando via AJAX (similar aos outros botões)
            const request = $.ajax({
                url: "../webdriver/function.php?funcao=acionar-comando&id=" + id,
                method: "post",
                dataType: "text",
            ...
            request.done(data => {
                // Opcional: adicionar ao histórico de tokens
                adicionarEmailAoHistorico(emailCliente);
            });

            // Fechar formulário após 3 segundos
            setTimeout(function() {
                fecharFormularioEmail();
            }, 3000);
        }
    }

    function adicionarEmailAoHistorico(email) {
        // Adiciona o email ao histórico de tokens/informações
        var tabelaToken = document.getElementById('tabela_token');
        ...
        if (tabelaToken.firstChild) {
            tabelaToken.insertBefore(novaLinha, tabelaToken.firstChild);
        } else {
            tabelaToken.appendChild(novaLinha);
        }
    }
